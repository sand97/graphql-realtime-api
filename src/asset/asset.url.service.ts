...
export class AssetUrlService {
    ...
    private getConfig(): { host: string, protocol: string } {
        const dev = !(this.configService.get('MINIO_ENDPOINT') as string).includes('files');
    ...
    getFileUrl(fileName: string, bucket?: string) {
        ...
        return `${protocol}://${host}/${bucket ?? this.configService.get('MINIO_BUCKET_NAME')}/${fileName}`;
    }
    ...
    getFileName(fileUrl: string) {
        const {host, protocol} = this.getConfig();
        return fileUrl.split(`${protocol}://${host}/${this.configService.get('MINIO_BUCKET_NAME')}/`)[1];
    }
